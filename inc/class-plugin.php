...
class Plugin {
	...
	public function admin_init() {
		...
		$fields = array(
			'wp_china_no_setting' => array(
				array(
					'name'    => 'super_store',
					'label'   => __( '市场加速', 'wp-china-no' ),
				...
				array(
					'name'    => 'super_admin',
					'label'   => __( '后台静态加速', 'wp-china-no' ),
					'desc'    => __( '替换 WordPress 所依赖的静态文件使用 WP-China-No 的公共节点，此选项可显著加快小带宽服务器的管理后台访问速度',
						'wp-china-no' ),
					'type'    => 'radio',
					'default' => 'on',
					'options' => array(
				...
				array(
					'name'    => 'super_gfont',
					'label'   => __( '谷歌字体加速', 'wp-china-no' ),
					'desc'    => __( '替换谷歌字体文件使用 WP-China-No 的公共节点，建议只在包含谷歌字体的情况下才启用该选项',
						'wp-china-no' ),
					'type'    => 'radio',
					'default' => 'off',
					'options' => array(
				...
				array(
					'name'    => 'super_gajax',
					'label'   => __( '谷歌前端公共库加速', 'wp-china-no' ),
					'desc'    => __( '替换谷歌前端公共库文件使用 WP-China-No 的公共节点，建议只在包含谷歌前端公共库的情况下才启用该选项',
						'wp-china-no' ),
					'type'    => 'radio',
					'default' => 'off',
					'options' => array(
				...
				array(
					'name'    => 'super_cdnjs',
					'label'   => __( 'CDNJS公共库加速', 'wp-china-no' ),
					'desc'    => __( '替换 CDNJS 公共库文件使用 WP-China-No 的公共节点，建议只在包含 CDNJS 公共库的情况下才启用该选项',
						'wp-china-no' ),
					'type'    => 'radio',
					'default' => 'off',
					'options' => array(
	...
	public function admin_menu() {
		...
		add_submenu_page(
			...
			esc_html__( 'WP-China-No', 'wp-china-no' ),
			esc_html__( 'WP-China-No', 'wp-china-no' ),
		...
		add_filter( 'plugin_action_links', function ( $links, $file ) {
			if ( 'wp-china-no/wp-china-no.php' !== $file ) {
				return $links;
			}
			$settings_link = '<a href="' . add_query_arg( array( 'page' => 'wp-china-no' ),
	...
	public static function deactivate() {
		// 清除设置
		delete_option( 'wp_china_no_setting' );
		delete_option( 'wp_china_no_about' );
	}
	...
	public static function check() {
		...
		if ( is_plugin_active( 'wp-china-yes/wp-china-yes.php' ) ) {
			...
			$notices[] = '<div class="notice notice-error is-dismissible">
					<p><strong>' . __( '检测到不兼容的插件 WP-China-Yes，已自动禁用！', 'wp-china-no' ) . '</strong></p>
				</div>';
		}
		if ( is_plugin_active( 'kill-429/kill-429.php' ) ) {
			...
			// 输出提示信息
			$notices[] = '<div class="notice notice-error is-dismissible">
					<p><strong>' . __( '检测到不兼容的插件 Kill 429，已自动禁用！', 'wp-china-no' ) . '</strong></p>
				</div>';
		...
		if ( defined( 'WP_PROXY_HOST' ) || defined( 'WP_PROXY_PORT' ) ) {
			...
			$notices[] = '<div class="notice notice-warning is-dismissible">
					...
					'wp-china-no' ) . '</strong></p>
				</div>';
		}

		set_transient( 'wp-china-no-admin-notices', $notices, 10 );
	}
	...
	public function admin_notices() {
		$notices = get_transient( 'wp-china-no-admin-notices' );
		if ( $notices ) {
			foreach ( $notices as $notice ) {
				echo $notice;
			}
			delete_transient( 'wp-china-no-admin-notices' );
		}
	}
}
