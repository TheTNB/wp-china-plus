...
class Super {
	...
	public function filter_wordpress_org( $preempt, $args, $url ) {
		...
		if ( $this->setting->get_option( 'super_store', 'wp_china_plus_setting', 'proxy' ) == 'super' ) {
			$url = str_replace( 'wpa.cdn.haozi.net/plugins/update-check/1.1', 'wepublish.cn/api/wp-json/plugin/v1/update-check', $url );
			$url = str_replace( 'wpa.cdn.haozi.net/themes/update-check/1.1', 'wepublish.cn/api/wp-json/theme/v1/update-check', $url );
			$url = str_replace( 'wpa.cdn.haozi.net/core/version-check/1.7', 'wepublish.cn/api/wp-json/core/v1/version-check', $url );
			$url = str_replace( 'wpa.cdn.haozi.net/plugins/info/1.2', 'wepublish.cn/api/wp-json/plugin/v1/info', $url );
